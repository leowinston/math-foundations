...
\begin{proof}
    Let $x \in \R$. For this proof, we will show the contrapositive, that is, if $x$ is rational, then $x^2$ is rational. Assume $x \in \mathbb{Q}$. Then, by definition of a rational number, $\exists p,q \in \mathbb{Z}$ such that $x = \frac{p}{q}$, where $q \ne 0$:
    \begin{align*}
    x&=\frac{p}{q}
    \end{align*}
...
\begin{proof}
Let $A, B, C$ be arbitrary sets. For this proof, we will prove the contrapositive, that is, $B \subseteq C$ implies that $A-C \subseteq A-B$. Assume $B \subseteq C$, and suppose $x$ is an arbitrary element of $A - C$. By definition of set difference, $x \in A - C$ implies that $x \in A$ and $x \not \in C$. Since $B \subseteq C$, it follows that $x \not \in C$ implies $x \not \in B$, by definition of subset. Thus, $x \not \in B$.
By definition of set difference, since $x \in A$ and $x \not \in B$, $x \in A-B$. So, by definition of subset, $A - C \subseteq A - B$. Thus, we've proven the contrapositive. Therefore, if $A - C \not\subseteq A - B$, then $ B \not\subseteq C$.
\end{proof}
